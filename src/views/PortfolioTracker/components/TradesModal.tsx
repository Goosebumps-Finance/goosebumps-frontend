        ...
        <Modal show size="xl" onHide={hide}>
            <div className="bg-dark border border-info">
                <Modal.Header className="border-info">
                    ...
                    <Button variant="secondary" onClick={hide} style={styles.btnStyle}><i className="fa fa-times" /></Button>
                </Modal.Header>
                        ...
                        <tbody className="border-top-0">
                            {token.trades.map((trade, index) =>
                                <tr key={JSON.stringify(trade)}>
                                    ...
                                    <td>
                                        <NumberFormat value={trade.tokenAmount} decimalScale={token.info.tokenscale} displayType="text" thousandSeparator="," />
                                    </td>
                                    <td>
                                        {
                                            trade.transactionType === 3 || trade.transactionType === 4
                                                ? <div className="input-group" style={{width: 130}}>
                                                    <NumberFormat
                                                        ...
                                                        onChange={e => {
                                                            ...
                                                            localStorage.setItem(`price_${trade.tx}`, `${parseFloat(e.target.value.replace(/[^0-9.]/g, ""))}`)
                                                        }}
                                                        thousandSeparator=","
                                                        prefix="$"
                                                    />
                                                ...
                                                : <NumberFormat value={trade.priceUSD} decimalScale={token.info.pricescale} displayType="text" thousandSeparator="," prefix="$" />
                                        }
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        {
                                            isBuyOrIn(trade)
                                                ? <>-</>
                                                : <NumberFormat value={trade.avarageBuyPriceOfHoldings} decimalScale={token.info.pricescale} displayType="text" thousandSeparator="," prefix="$" />
                                        }
                                    </td>
                                    <td>
                                        {
                ...
                <Modal.Footer className="border-info">
                    <Button variant='secondary' style={styles.btnStyle} onClick={hide}>Close</Button>
                </Modal.Footer>
            </div>
        </Modal>
    );
