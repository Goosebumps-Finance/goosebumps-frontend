import { Button, Flex, Heading } from '@goosebumps/uikit'
...
const HarvestAction: React.FC<FarmCardActionsProps> = ({ earnings, pid, rewardSymbol }) => {
  ...
  const empirePrice = usePriceEmpireBusd()
  const dispatch = useAppDispatch()
  const rawEarningsBalance = account ? getBalanceAmount(earnings) : BIG_ZERO
  const displayBalance = rawEarningsBalance.toFixed(3, BigNumber.ROUND_DOWN)
  // const earningsBusd = rawEarningsBalance ? rawEarningsBalance.multipliedBy(cakePrice).toNumber() : 0
  const earningsBusd = rawEarningsBalance ? rawEarningsBalance.multipliedBy(empirePrice).toNumber() : 0
