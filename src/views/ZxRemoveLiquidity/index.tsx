...
import { useZxPairContract } from '../../hooks/useContract'
...
import { useBurnActionHandlers, useZxDerivedBurnInfo, useBurnState } from '../../state/burn/hooks'
...
export default function ZxRemoveLiquidity({
  history,
  match: {
  ...
  const { pair, parsedAmounts, error } = useZxDerivedBurnInfo(currencyA ?? undefined, currencyB ?? undefined)
  ...
  // pair contract
  const pairContract: Contract | null = useZxPairContract(pair?.liquidityToken?.address)
  ...
  const handleSelectCurrencyA = useCallback(
    (currency: Currency) => {
      if (currencyIdB && currencyId(currency) === currencyIdB) {
        history.push(`/liquidityRemove/${currencyId(currency)}/${currencyIdA}`)
      } else {
        history.push(`/liquidityRemove/${currencyId(currency)}/${currencyIdB}`)
      }
    },
  ...
  const handleSelectCurrencyB = useCallback(
    (currency: Currency) => {
      if (currencyIdA && currencyId(currency) === currencyIdA) {
        history.push(`/liquidityRemove/${currencyIdB}/${currencyId(currency)}`)
      } else {
        history.push(`/liquidityRemove/${currencyIdA}/${currencyId(currency)}`)
      }
    },
    [currencyIdA, currencyIdB, history],
  )
                ...
                <LightGreyCard>
                  ...
                  {chainId && (oneCurrencyIsWETH || oneCurrencyIsETH) ? (
                    <RowBetween style={{ justifyContent: 'flex-end', fontSize: '14px' }}>
                      {oneCurrencyIsETH ? (
                        <StyledInternalLink
                          to={`/liquidityRemove/${currencyA === ETHER ? WETH[chainId].address : currencyIdA}/${currencyB === ETHER ? WETH[chainId].address : currencyIdB
                            }`}
                        >
                          {t('Receive WBNB')}
                        </StyledInternalLink>
                        ...
                        <StyledInternalLink
                          to={`/liquidityRemove/${currencyA && currencyEquals(currencyA, WETH[chainId]) ? 'BNB' : currencyIdA
